...
public class Util {
    ...
    /**
     * 不论是快照还是事务日志,其命名都是 prefix.(zxid),通过此方法可以获取文件的zxid(只需修改 prefix 参数即可)
     * <p>
     * 快照的zxid含义:从[1,zxid]直接的所有事务的执行结果都保存在快照文件中
     * 事务日志的zxid含义:本事务日志文件存储的第一个事务的zxid即为文件后缀名
     * <p>
    ...
    /**
     * 判断文件是否是有效的快照.若在执行快照时服务器宕机,则因为快照文件不完整而无效;
     * 若文件不是快照文件,自然也是无效的
     * <p>
    ...
    public static List<File> sortDataDir(File[] files, String prefix, boolean ascending) {
        ...
        filelist.sort(new DataDirFileComparator(prefix, ascending));
        return filelist;
    }
